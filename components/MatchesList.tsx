...
const TABS: { key: TabKey; label: string; emoji: string; color: string }[] = [
  { key: 'group',        label: 'Grupos',   emoji: '⚽', color: 'text-purple-400  border-purple-500' },
  { key: 'ro32',         label: 'R32',      emoji: '🔥', color: 'text-orange-400  border-orange-500' },
  { key: 'ro16',         label: 'Octavos',  emoji: '⚡', color: 'text-yellow-400  border-yellow-500' },
  { key: 'quarterfinal', label: 'Cuartos',  emoji: '💥', color: 'text-red-400     border-red-500'    },
  { key: 'semifinal',    label: 'Semis',    emoji: '🌟', color: 'text-pink-400    border-pink-500'   },
  { key: 'final',        label: 'Final',    emoji: '👑', color: 'text-yellow-300  border-yellow-400' },
]

function defaultTab(nextMatch?: Match | null): TabKey {
  if (!nextMatch) return 'group'
  const stage = nextMatch.stage as TabKey
  // third_place va dentro de la tab "final"
...
export default function MatchesList({ matches, predictions, isLoggedIn, nextMatch }: MatchesListProps) {
  ...
  // Cantidad de pronósticos pendientes por tab (partido no cerrado sin pronóstico)
  const pendingByTab = (tab: TabKey) => {
    ...
    return matchesByTab[tab].filter(m => {
      ...
      const hasPred = predictionsMap.has(m.id)
      return !closed && !finished && !hasPred
    }).length
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* PRÓXIMO PARTIDO — siempre visible arriba */}
      {nextMatch && (
        <section>
          <h2 className="text-2xl font-bold text-purple-400 mb-4">⏰ Próximo Partido</h2>
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${
                  isActive
                    ? `bg-gray-800 border-b-2 ${tab.color}`
      ...
      <div className="space-y-8">
              ...
              return (
                <section key={group}>
                  <h3 className="text-xl font-bold mb-4 text-white flex items-center gap-2">
                    <span className="w-8 h-8 bg-purple-600/20 border border-purple-500/50 rounded-lg flex items-center justify-center text-purple-400 text-sm">
                      {group}
                    </span>
                    Grupo {group}
                  </h3>
        ...
        {activeTab === 'ro32' && (
          <section>
            <h2 className="text-3xl font-bold mb-6 text-orange-400">🔥 Ronda de 32</h2>
            <div className="flex flex-col items-center space-y-3">
        ...
        {activeTab === 'ro16' && (
          <section>
            <h2 className="text-3xl font-bold mb-6 text-yellow-400">⚡ Octavos de Final</h2>
            <div className="flex flex-col items-center space-y-3">
        ...
        {activeTab === 'final' && (
          <div className="space-y-8">
            ...
            {finalMatch.length > 0 && (
              <section className="border-4 border-yellow-500 bg-gradient-to-br from-yellow-900/20 to-yellow-700/10 rounded-2xl p-6">
